...
def test_pi_parameters_response_validates_timeseries_parameters():
    ...
    assert [parameter.id for parameter in result] == ["alpha", "enabled"]
...
def test_pi_filters_response_validates_nested_children_and_bounding_boxes():
    ...
    assert len(result) == 1
    assert result[0].id == "Viewer"
    assert [filter_.id for filter_ in result] == ["Viewer"]


def test_pi_locations_response_can_be_used_as_location_sequence():
    payload = {
        "version": "1.34",
        "geoDatum": "WGS 1984",
        "locations": [
    ...
    assert len(result) == 1
    assert result[0].location_id == "Amanzimtoti_River_level"
    assert [location.location_id for location in result] == ["Amanzimtoti_River_level"]
